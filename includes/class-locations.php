...
class Locations
{
    ...
    */
    public function load_states()	{		$country_id = isset($_POST['country_id']) ? intval($_POST['country_id']) : 0;		if ($country_id) {			$query = "SELECT id, name FROM states WHERE country_id = :country_id ORDER BY name ASC";			$states = $this->query($query, ['country_id' => $country_id]);						if ($states) {				wp_send_json_success($states);			} else {				wp_send_json_error(['message' => __('No states found.', 'DIRECTORYPRESS')]);			}		} else {			wp_send_json_error(['message' => __('Invalid country ID.', 'DIRECTORYPRESS')]);		}	}		/**     * Load cities based on the selected state ID.    */		public function load_cities()	{		$state_id = isset($_POST['state_id']) ? intval($_POST['state_id']) : 0;		if ($state_id) {			/* Select the city and use the id as the identifier */			$query = "SELECT id, city FROM zipcodes WHERE state_id = :state_id ORDER BY city ASC";			$cities = $this->query($query, ['state_id' => $state_id]);			if ($cities) {				/* Format city names */				foreach ($cities as &$city) {					$city_normalized = strtolower($city['city']); /* Convert to lowercase */					$city['city'] = ucwords($city_normalized); /* Capitalize properly */				}				wp_send_json_success($cities);			} else {				wp_send_json_error(['message' => __('No cities found.', 'DIRECTORYPRESS')]);			}		} else {			wp_send_json_error(['message' => __('Invalid state ID.', 'DIRECTORYPRESS')]);		}	}		/**     * Retrieve location details by listing ID.     * @param int $listing_id The ID of the listing (post_id).     * @return array|false Array with country, state, city, postal code, or false if not found.    */    public function get_location_by_listing_id($listing_id)	{		global $wpdb;		/* Table name encapsulated inside the function */		$table_name = $wpdb->prefix . 'directorypress_locations_relation';		/* Ensure the listing ID is a valid integer */		$listing_id = (int) $listing_id;		/* Prepare the SQL query to fetch location data including location_id and city_id */		$query = $wpdb->prepare(			"SELECT location_id, state_id, city_id, country, state, city, zip_or_postal_index 			 FROM $table_name 			 WHERE post_id = %d LIMIT 1",			$listing_id		);		/* Execute the query and retrieve the results */		$location = $wpdb->get_row($query, ARRAY_A);		/* Return the location details or false if no results found */		if ($location)		{			/* Ensure IDs are integers */			$location['country_id'] = (int) $location['location_id'];			$location['state_id'] = (int) $location['state_id'];			$location['city_id'] = (int) $location['city_id'];			unset($location['location_id']); /* Remove the original location_id key */			return $location;		}		else		{			return false;		}	}		/**	 * Load states based on the provided country ID.	 * @param int $country_id The ID of the country for which states are to be loaded.	 * @return array The states corresponding to the given country ID.	*/	public function get_states_by_country_id($country_id)	{		/* Ensure the country ID is an integer */		$country_id = (int) $country_id;		if ($country_id > 0) {			/* Prepare the query to fetch states */			$query = "SELECT id, name FROM states WHERE country_id = :country_id";						/* Execute the query and get the states */			$states = $this->query($query, ['country_id' => $country_id]);			/* Ensure IDs are integers */			foreach ($states as &$state) {				$state['id'] = (int) $state['id'];			}			if ($states) {				return array('success' => true, 'data' => $states);			} else {				return array('success' => false, 'message' => __('No states found.', 'DIRECTORYPRESS'));			}		} else {			return array('success' => false, 'message' => __('Invalid country ID.', 'DIRECTORYPRESS'));		}	}		/**	 * Get cities by state ID.	 * @param int $state_id The ID of the state.	 * @return array An array with success status and data or an error message.	*/	public function get_cities_by_state_id($state_id)	{		/* Ensure the state ID is an integer */		$state_id = (int) $state_id;		if ($state_id > 0) {			/* Prepare the query to fetch cities based on the state ID */			$query = "SELECT id, city FROM zipcodes WHERE state_id = :state_id";						/* Execute the query and fetch cities */			$cities = $this->query($query, ['state_id' => $state_id]);			if ($cities) {				/* Format city names: Capitalize properly and ensure IDs are integers */				foreach ($cities as &$city) {					$city_normalized = strtolower($city['city']); /* Convert to lowercase */					$city['city'] = ucwords($city_normalized); /* Capitalize properly */					$city['id'] = (int) $city['id']; /* Ensure the city ID is an integer */				}				return array('success' => true, 'data' => $cities);			} else {				return array('success' => false, 'message' => __('No cities found.', 'DIRECTORYPRESS'));			}		} else {			return array('success' => false, 'message' => __('Invalid state ID.', 'DIRECTORYPRESS'));		}	}
    /**
