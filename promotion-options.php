...
function add_marker_to_listing_on_order_completion($order_id)
{
    ...
    foreach ($order->get_items() as $item_id => $item) 
    {
        ...
        if ($listing_id) 
        {
            ...
            /* If no promotions were applied, mark it as a free listing
            if (!$has_promotion) 
            {
                update_post_meta($listing_id, 'promotion_level', '_is_free_listing');

                /* Retrieve individual validity period for a free listing from ACF
                $validity_period = get_field('validity_period', $listing_id);
                ...
                /* Set an expiration date for markers
                $expiry_date_utc = strtotime("+{$validity_period} days");
                $expiry_date = gmdate('d/m/Y H:i:s', $expiry_date_utc); /* Date in d/m/Y H:i:s format
				/* Update expiry result
                $update_expiry_result = update_post_meta($listing_id, '_marker_expiration_date', $expiry_date);
            }
        }
    }
}
